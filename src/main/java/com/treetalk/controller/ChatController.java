...
public class ChatController {
    ...
    public ApiResponse<ChatDto.SessionResponse> createSession(
    ...
    public ApiResponse<List<ChatDto.SessionResponse>> getUserSessions(
            @Parameter(hidden = true) @RequestAttribute("userId") Long userId) {
        List<ChatDto.SessionResponse> responses = chatService.getUserSessions(userId);
        return ApiResponse.success("获取会话列表成功", responses);
    }

    @GetMapping("/session/{sessionId}")
    @Operation(summary = "获取会话详情", description = "根据会话ID获取聊天会话的详细信息")
    public ApiResponse<ChatDto.SessionResponse> getSession(
            @Parameter(hidden = true) @RequestAttribute("userId") Long userId,
        ...
        return ApiResponse.success("获取会话成功", response);
    }
    ...
    public ApiResponse<List<ChatDto.MessageResponse>> getMessages(
            @Parameter(hidden = true) @RequestAttribute("userId") Long userId,
            @Parameter(description = "会话ID", required = true) @PathVariable Long sessionId,
        ...
        return ApiResponse.success("获取消息列表成功", responses);
    }
